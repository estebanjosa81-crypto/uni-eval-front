...
export function AdminSidebar({ isCollapsed = false, onToggle }: AdminSidebarProps) {
  ...
  useEffect(() => {
    const cargarUsuario = () => {
      try {
        ...
        // Usar el usuario del hook useAuth si está disponible
        if (authUser) {
          setUser(authUser as User)
        } else {
          // Obtener datos del usuario desde el servicio centralizado
  ...
  const handleLogout = async () => {
    try {
      ...
      // Llamar al servicio de logout (esto limpia los tokens automáticamente)
      ...
      // Redirigir a la página de login de forma forzada
      window.location.href = "/login"
    } catch (error) {
      console.error('Error al cerrar sesión:', error)
      
      // Aún así intentar limpiar tokens y redirigir
      authService.logout()
  ...
  // Función para obtener el rol principal a mostrar (usando lógica centralizada)
